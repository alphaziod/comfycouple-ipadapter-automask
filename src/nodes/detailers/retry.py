"""Retry-capable crop-aware Impact Pack detailer node."""
...
class SayaDetailerForEachAutoRetry:
    """Crop-aware Impact Pack detailer with retry support for rejected patches."""

    RETURN_TYPES = ("IMAGE",)
    FUNCTION = "doit"
    CATEGORY = "ImpactPack/Detailer"
    DESCRIPTION = (
        "Enhance each detected region and retry patches rejected by the active detailer hook."
    )

    @classmethod
    def INPUT_TYPES(cls: type[Self]) -> dict[str, Any]:
        """Return the ComfyUI input schema exposed by this node."""
        return build_detailer_input_schema(include_retry_limit=True)
    ...
    def doit(
        ...
        """Execute one detailer pass with a bounded retry policy."""
